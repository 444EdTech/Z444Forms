...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Header bar */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6 bg-slate-900 border border-slate-800 text-white rounded-3xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-505/10 rounded-full blur-2xl" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        {/* Total Registrations */}
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xl shadow-slate-100/30 flex items-center justify-between">
          <div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xl shadow-slate-100/30 md:col-span-3">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-400 block mb-4">Registration Distribution by B.Tech Year</span>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            ...
            {[
              ...
              return (
                <div key={idx} className="bg-slate-50/50 p-3 rounded-xl border border-slate-100 flex flex-col justify-between">
                  <div>
                    <p className="text-[10px] uppercase font-bold text-slate-400 leading-none">{y.label}</p>
                    <p className="text-lg font-bold text-slate-800 mt-1">{y.count}</p>
                  </div>
      ...
      {/* Main Table area */}
      <div className="bg-white border border-slate-100 rounded-2xl shadow-xl shadow-slate-100/30 overflow-hidden">
        
        {/* Search and Filters panel */}
        <div className="p-6 bg-slate-50/60 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative max-w-md w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 focus:border-indigo-500 focus:ring-indigo-100 rounded-lg text-xs font-semibold focus:outline-none transition-all"
            />
          </div>
            ...
            <div className="flex bg-slate-200/40 p-1 rounded-lg">
              {["All", "1", "2", "3", "4", "Completed"].map((year) => (
                <button
                  ...
                  className={`px-3 py-1 text-[11px] font-bold rounded-md cursor-pointer transition-all ${selectedYear === year ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
                >
        ...
        {/* Error message */}
        {error && (
          <div className="p-6 text-center text-rose-500 bg-rose-50/50 border-b border-rose-100 text-xs font-semibold">
            {error}
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/20 text-slate-400 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100">
                ...
                {filteredRegistrations.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-slate-50/20 transition-all duration-150">
                    <td className="p-4 pl-6">
                      ...
                      <div className="text-[11px] text-slate-400 mt-0.5 font-mono">{item.id}</div>
                    </td>
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold ${
                        item.btechYear === "1" ? "bg-blue-50 text-blue-700 border border-blue-100" :
                        item.btechYear === "2" ? "bg-emerald-50 text-emerald-700 border border-emerald-100" :
                        item.btechYear === "3" ? "bg-indigo-50 text-indigo-700 border border-indigo-100" :
                        item.btechYear === "4" ? "bg-violet-50 text-violet-700 border border-violet-100" :
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-bold uppercase text-slate-800">{item.department}</span>
                      </div>
                    </td>
        ...
        <div className="p-5 bg-slate-50/50 border-t border-slate-100 flex flex-wrap justify-between items-center gap-3 text-[11px] text-slate-400 font-semibold uppercase tracking-wider">
